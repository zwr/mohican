...
namespace :seed do
  ...
  task activities: :environment do
    ...
    resp3 = RestClient.get("http://#{address}/Celesta/z/activities/layout", cookies: resp.cookies, accept: :json)
    hash_response = JSON.parse(resp3.body)
    resp_string = JSON.pretty_generate hash_response
    resp_string.force_encoding('UTF-8')
    File.open(Rails.root.join('db','seeds','activities_layout.json'), 'w') {|f| f.write(resp_string) }
